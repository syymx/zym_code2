...
def _deserialize_model(f, custom_objects=None, compile=True):
    ...
    if 'keras_version' in model_weights_group:
        original_keras_version = model_weights_group['keras_version'].encode('utf8')
    else:
        original_keras_version = '1'
    if 'backend' in model_weights_group:
        original_backend = model_weights_group['backend'].encode('utf8')
    else:
    ...
    for name in layer_names:
        ...
        if isinstance(weight_names, list):
            if weight_names:
                filtered_layer_names.append(name)
        elif weight_names.any():
            filtered_layer_names.append(name)
